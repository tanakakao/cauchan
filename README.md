...
- SCM、DoWhy Linear Regression、LinearDML、CausalForestDMLによる平均因果効果推定
- 最終構造に含まれる全有向エッジの一括推定と一覧・ヒートマップ表示
...
必須エッジが最終構造に存在しない場合は警告として表示しますが、推論は継続できます。
...
- `SCM`: DoWhyを使わない親変数調整付き線形回帰
- `DoWhyLinearRegression`: DoWhyで識別後、backdoor linear regression
- `LinearDML`: DoWhyで識別後、EconML LinearDML
- `CausalForestDML`: EconMLで異質的効果を学習し、その平均を返す因果フォレスト

個別の変数ペアだけでなく、最終構造に存在する全有向エッジを一括推定し、一覧またはヒートマップで確認できます。
...
- 循環、禁止条件との競合、未方向辺の残存がある場合は推論を実行できません。
- 必須エッジ不足は警告として表示しますが、推論は実行できます。
...
- CausalForestDMLには介入変数の非子孫となるベースライン変数と20行以上のデータが必要です。
- CausalForestDMLを全エッジへ適用すると、エッジ数に応じて計算時間が増加します。
